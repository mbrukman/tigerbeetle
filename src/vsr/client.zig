...
pub fn Client(comptime StateMachine_: type, comptime MessageBus: type) type {
    return struct {
        ...
        registering: ?*Message.Request = null,
        ...
        pub fn deinit(self: *Self, allocator: std.mem.Allocator) void {
            if (self.registering) |message| self.release(message.base());
            if (self.inflight) |inflight| self.release(inflight.message.base());
        ...
        pub fn inflight_message(self: *const Self) ?*Message.Request {
            if (self.registering) |message| return message;
        ...
        pub fn raw_request(
            ...
            // Register before trying to start the inflight message.
            self.register();
        ...
        fn on_request_timeout(self: *Self) void {
            ...
            const message = self.inflight_message().?;
            assert(message.header.command == .request);
        ...
        fn send_request_for_the_first_time(self: *Self, message: *Message.Request) void {
            assert(message == self.inflight_message().?);
